...
def studentview(request):
    ...
    if admininfo.objects.filter(email=request.session['unid']).exists():
        studentstable=studentevent.objects.all()
        studentstable2=studentinfo.objects.all()
        return render(request,"adminstudentdisaplay.html",{'displaydata':studentstable,'stdinfo':studentstable2})
...
def search(request):
    if admininfo.objects.filter(email=request.session['unid']).exists():
        ...
        studentstable=studentevent.objects.filter(event=searchtext)
        studentstable2=studentinfo.objects.all()
        return render(request,"adminstudentdisaplay.html",{'displaydata':studentstable,'stdinfo':studentstable2})
